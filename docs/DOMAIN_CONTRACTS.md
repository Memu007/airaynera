...
- Bandeja fuera del modal: el dashboard muestra un panel `Trabajo sin confirmar` que lee los mismos registros por pestaña **y por cuenta** de `localStorage` (`aira:conflict:<cuenta>:<id>:<tab>` y `aira:pending:<cuenta>:<id>:<tab>`, con `<cuenta>` = `sub` del JWT) y lista cada sesión con un conflicto pendiente o un borrador sin confirmar. El trabajo local de una cuenta nunca se lee bajo otra cuenta en la misma pestaña; al cerrar sesión se limpia el estado en memoria y la bandeja. Un conflicto y un borrador de la misma sesión se muestran una sola vez (gana el conflicto). Un borrador cuyo guardado sigue en vuelo en la pestaña no aparece. Un registro cuya sesión fue borrada o es inaccesible no produce una tarjeta muerta: aparece como `Sesión no disponible` sólo con acción de descartar. Cada entrada accesible abre la ficha en su vista de conflicto o recuperación; la bandeja persiste tras recargar, se re-renderiza al crear o resolver trabajo (éxito, "Usar la del servidor", descartar, agotar reintentos) y se oculta cuando no queda nada pendiente. Tras un reintento exitoso de recuperación, la entrada y la bandeja desaparecen sin recargar. Nombres y notas se renderizan como texto.
