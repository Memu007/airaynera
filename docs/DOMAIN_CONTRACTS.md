...
- Bandeja fuera del modal: el dashboard muestra un panel `Trabajo sin confirmar` que lee los mismos registros por pestaña de `localStorage` (`aira:conflict:<id>:<tab>` y `aira:pending:<id>:<tab>`) y lista cada sesión con un conflicto pendiente o un borrador sin confirmar. Un conflicto y un borrador de la misma sesión se muestran una sola vez (gana el conflicto). Un borrador cuyo guardado sigue en vuelo en la pestaña no aparece. Cada entrada abre la ficha en su vista de conflicto o recuperación; la bandeja persiste tras recargar, se re-renderiza al crear o resolver trabajo (éxito, "Usar la del servidor", descartar, agotar reintentos) y se oculta cuando no queda nada pendiente. Nombres y notas se renderizan como texto.
