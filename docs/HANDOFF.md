...
- Operación de agentes: `AGENTS.md` fija el orden de lectura, el tono objetivo, la obligación de separar evidencia de hipótesis y una plantilla de un solo hito para darle instrucciones verificables a la dev.
...
- Concurrencia de edición con garantía real, no sólo de pantalla: las sesiones tienen una columna `revision`; `PATCH /api/sessions/:id` **exige** `If-Match` (falta → `428`, malformada o no entera segura → `400`) y actualiza de forma condicional, devolviendo `409` con la sesión vigente ante una edición concurrente. Un cuerpo vacío/ignorado da `400 EMPTY_PATCH` sin tocar `revision`. En el cliente, cada edición tiene una **secuencia monotónica**: sólo el payload más nuevo sobrevive/envía (un único `pump` es el emisor), así ni siquiera tras agotar los reintentos de una respuesta perdida un payload viejo puede ganarle a uno nuevo (al rendirse, la versión más nueva se guarda en `localStorage` y no queda `pending` con `inFlight=false`). El formulario **no se cierra hasta confirmar**; ante `409` se muestra un panel de conflicto **campo por campo** (servidor vs mía) que se **persiste** (reabrir la ficha o recargar la página lo vuelve a mostrar), con **Reintentar** (merge de 3 vías: envía sólo los campos que cambié, sin pisar los que tocó el otro cliente), **Editar mi versión** o **Usar la del servidor**; las acciones se deshabilitan durante un reintento en curso. Una respuesta perdida se reconcilia consultando la sesión y, si ya se aplicó, adoptando su revisión y enviando la pendiente. Se advierte (`beforeunload`) antes de cerrar/recargar con formulario modificado o conflicto pendiente.
...
- `scripts/session-edit-browser-tests.js` (Chromium por `playwright-core`, fuera de `npm test`) aprobó 64/64: además de edición con recarga, borrado de medicación, rechazos de formulario, filtros, teclado y render literal, cubre la concurrencia real: la versión más nueva gana sobre una respuesta demorada; serialización ante una primera solicitud lenta; **respuesta perdida** (aplicada pero cortada con `route.fetch()`+abort) que termina en la última versión; **recuperación agotada** (5 PATCH fallidos + guardar v3 → base e interfaz terminan en v3, sin que v1/v2 ganen); `409` de otro cliente que muestra ambas versiones, **no** borra mi texto y su reintento persiste; `409` con edición pendiente que conserva la pendiente; **conflicto multicampo** que lista los 8 campos, sobrevive cerrar/reabrir y recargar, deshabilita las acciones durante el reintento, hace merge de 3 vías (no pisa el campo del otro cliente) y ofrece "Usar la del servidor"; y una respuesta tardía que no descarta cambios sin guardar. El runner resuelve el navegador por `PLAYWRIGHT_CHROMIUM_PATH`/`CHROMIUM_PATH`, instalación del sistema (macOS/Linux) o canal `chrome`, y sirve los assets CDN desde `vendor/`. Corre en un job de CI en ubuntu.
...
- Verificación del hito actual: `npm test` con 129/129 funcionales + 130/130 de edición, navegador real 64/64, smoke del supervisor, build, contrato UI, sintaxis embebida y `git diff --check` aprobados.
...
6. Ejecutar `npm test`, `npm run test:session-edit:browser`, `npm run test:runtime-supervisor`, `npm run build` y `npm run lint` antes de cada publicación; la batería actual aprueba 129/129 + 130/130 y 64/64 en navegador.
